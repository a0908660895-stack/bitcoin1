...
class PruneLockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_array_result(locks, {"name": "rpc:testlock"}, {"height": 200})
        assert_array_result(locks, {"name": "rpc:another"}, {"height": 300})
        ...
        assert_equal(len(locks), 1)
        assert_array_result(locks, {"name": "rpc:another"}, {"height": 300})
        assert_array_result(locks, {"name": "rpc:testlock"}, {}, should_not_find=True)
        ...
        self.log.info("Test empty lock name fails")
        assert_raises_rpc_error(-8, "Prune lock name must not be empty", node.setprunelock, "", "add", 100)
