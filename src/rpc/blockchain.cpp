    ...
    return RPCHelpMan{"listprunelocks",
        ...
        RPCResult{
            RPCResult::Type::ARR, "", "",
            {
                {RPCResult::Type::OBJ, "", "",
                {
                    {RPCResult::Type::STR, "name", "The identifier of the prune lock"},
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    {
        LOCK(::cs_main);
        const auto& prune_locks = chainman.m_blockman.GetPruneLocks();
        for (const auto& [name, lock_info] : prune_locks) {
            UniValue lock_obj(UniValue::VOBJ);
            lock_obj.pushKV("name", name);
...
static RPCHelpMan setprunelock()
{
    return RPCHelpMan{"setprunelock",
        ...
        {
            {"name", RPCArg::Type::STR, RPCArg::Optional::NO, "A unique identifier for this prune lock. Internally stored with an \"rpc:\" prefix to avoid collisions with system-managed locks."},
    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (name.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Prune lock name must not be empty");
    }

    const std::string command{request.params[1].get_str()};

    // Prefix RPC-created locks to avoid collision with internal lock names
    const std::string lock_name{"rpc:" + name};
    ...
    if (command == "add") {
        ...
        LOCK(::cs_main);
        chainman.m_blockman.UpdatePruneLock(lock_name, lock_info);
    } else if (command == "remove") {
        LOCK(::cs_main);
        const bool existed{chainman.m_blockman.DeletePruneLock(lock_name)};
        if (!existed) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                strprintf("Prune lock \"%s\" not found", name));
        }
    } else {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
