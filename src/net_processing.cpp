        ...
        const CBlockIndex& last_header, bool received_new_header, bool may_have_more_headers)
{
    ...
    if (m_chainman.IsInitialBlockDownload() && !may_have_more_headers) {
        ...
        // their tip.
        if (m_blockdownloadman.GetBestKnownBlock(pfrom.GetId()) && m_blockdownloadman.GetBestKnownBlock(pfrom.GetId())->nChainWork < m_chainman.MinimumChainWork()) {
    ...
    if (!pfrom.fDisconnect && pfrom.IsFullOutboundConn() && m_blockdownloadman.GetBestKnownBlock(pfrom.GetId()) != nullptr) {
        if (m_outbound_peers_with_protect_from_disconnect < MAX_OUTBOUND_PEERS_TO_PROTECT_FROM_DISCONNECT && m_blockdownloadman.GetBestKnownBlock(pfrom.GetId())->nChainWork >= m_chainman.ActiveChain().Tip()->nChainWork && !nodestate->m_chain_sync.m_protect) {
        ...
        m_connman.ForNode(to_disconnect, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (node_state == nullptr ||
                ...
                LogDebug(BCLog::NET, "keeping block-relay-only peer=%d chosen for eviction (connect time: %d, blocks_in_flight: %d)\n",
                         pnode->GetId(), count_seconds(pnode->m_connected), m_blockdownloadman.GetBlocksInFlight(pnode->GetId()).size());
            }
            return false;
        });
    }
    ...
    if (m_connman.GetExtraFullOutboundCount() > 0) {
        ...
        if (worst_peer != -1) {
            bool disconnected = m_connman.ForNode(worst_peer, [&](CNode* pnode) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
                ...
                // block from.
                if (now - pnode->m_connected > MINIMUM_CONNECT_TIME && m_blockdownloadman.GetBlocksInFlight(pnode->GetId()).empty()) {
...
bool PeerManagerImpl::SendMessages(CNode& node)
{
...

    {
        ...
        if (!m_blockdownloadman.GetBlocksInFlight(node.GetId()).empty()) {
            const node::QueuedBlock &queuedBlock = m_blockdownloadman.GetBlocksInFlight(node.GetId()).front();
        ...
        //
        std::vector<CInv> vGetData;
        if (CanServeBlocks(peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(peer)) || !m_chainman.IsInitialBlockDownload()) && m_blockdownloadman.GetBlocksInFlight(node.GetId()).size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            std::vector<const CBlockIndex*> vToDownload;
            NodeId staller = -1;
            auto get_inflight_budget = [this, &node]() EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
                return std::max(0, MAX_BLOCKS_IN_TRANSIT_PER_PEER - static_cast<int>(m_blockdownloadman.GetBlocksInFlight(node.GetId()).size()));
            };
            ...
            if (m_blockdownloadman.GetBlocksInFlight(node.GetId()).empty() && staller != -1) {
